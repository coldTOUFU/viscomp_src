class Movements {
    ...
    protected void rotateHip(float xAng, float yAng, float zAng) {
        rotateX(xAng); rotateY(yAng); rotateZ(zAng);
        translate(0, .25, 0); // hip: box(1., .5, .75);. called place will be bottom of hip.
        hip();
    }
    
    protected void rotateLimb(float xAngUpper, float yAngUpper, float zAngUpper, float xAngFore, float yAngFore, float zAngFore) {
